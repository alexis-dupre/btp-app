...
- **`docs/product/sources/costructor-devis-spec.md` §5.1** is what the competitor *collects*:
  a three-tab modal with accounting, CRM and multi-value fields.
- **The two real devis** are what SoRenov *prints*: a name and a postal address. Nothing else.
...
| `N° SIREN/SIRET` | collected | absent | **optional** | R-001 (CONFIRMED) makes the client's identification part of a structured invoice. The field must exist before E4 and costs nothing to fill early. A rule drafted as R-009 and awaiting entry in the register finds the mandatory mention to be the **SIREN**, nine digits, not the SIRET — do not narrow this field to a SIRET shape. It stays optional on the contact either way; the blocking validation is E4's, at issuance. See SPEC.md |
...
### Tab *Comptabilité* — none of it in E0
